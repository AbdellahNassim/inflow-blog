...
function BlogList({ posts, searchParams }: Props) {
      ...
      posts.filter((post) => {
        ...
        if (searchParams?.categories) {
          const categories = searchParams.categories
            .split(",")
            .map((category: string) => category.trim().toLocaleLowerCase());
          containsCategory = post.categories.some(
            (category) =>
              categories.indexOf(category.title.toLowerCase()) !== -1
          );
        }
        return containsSearch && containsCategory;
      }),
    [searchParams, posts]
  );
  return (
    <div className="mt-24">
      ...
      {_posts.length === 0 && (
        <div className="mx-auto grid h-full w-full max-w-lg place-items-center">
          <svg
        ...
        {_posts.map((post) => (
          <ClientSideRoute key={post._id} route={`/post/${post.slug.current}`}>
            <div className="group flex cursor-pointer flex-col">
              <div className="relative h-80 w-full rounded-xl drop-shadow-xl transition-transform duration-200 ease-out group-hover:scale-105">
                <Image
                  ...
                  fill
                />
                <div className="absolute bottom-0 flex w-full justify-between rounded-xl bg-black bg-opacity-20 p-5 text-white drop-shadow-lg backdrop-blur-lg">
                  <div>
                  ...
                  <div className="flex flex-col items-center gap-y-2 md:flex-row md:gap-x-2">
                    {post.categories.map((category) => (
                      <div
                        className="w-min rounded-full bg-[#E7E247] px-3 py-1 text-center font-semibold text-black"
                        key={category._id}
                      >
