import {addDoc, collection, getDocs, query, where, orderBy, getDoc,doc,onSnapshot, CollectionReference, updateDoc, serverTimestamp} from "firebase/firestore"
...
export const useComments = (postSlug: string) => {
    ...
    const commentsRef = collection(firestore, 'comments') as CollectionReference<Comment>
    const usersRef = collection(firestore, 'users') as CollectionReference<User>
    const onSnapshotComments = async () => {
        const queryComments = query(commentsRef, where('postSlug', '==', postSlug), orderBy('upvotes', 'desc'))
    ...
    const createNewComment = async (email: string, comment : string,postSlug : string) =>{
    ...
    const fetchComments = async () => {
        ...
        const authors = await Promise.all(data.docs.map((document) => {
    ...
    return { comments, fetchComments,upvoteComment,onSnapshotComments,unupvoteComment,createNewComment, loading }
}